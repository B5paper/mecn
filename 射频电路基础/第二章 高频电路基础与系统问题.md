...
为了区别不同电路中电阻的位置，下面与电感L并联的电阻都用大写字母R表示，而与电感串联的电阻，都用小写字母r表示。
...
$$Q = \omega_0 R C = \frac R {\omega_0 L}$$ （其中R为并联电阻）
...
谐振回路没有负载时，可以用$$W_{0.7} = \frac {f_0}{Q_0}$$来计算通频带。谐振回路另外还带有负载$$R_L$$时，需要用$$W_{0.7} = \frac {f_0}{Q_L}$$来计算通频带。
...
设理想变压器初级线圈匝数为$N_1$，次级线圈匝数为$N_2$，则次级回路上的电阻$R_L$折合到初级回路时的等效电阻$$R_L' = \left( \frac {N_1}{N_2} \right)^2 R_L $$

若初级线圈中心抽头，则$$R'_L = (\frac {2N_1}{N_2})^2 R_L$$，每个初级回路分到的电阻均为$$\frac 1 2 R'_L$$ 。
